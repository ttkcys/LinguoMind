...
import 'package:linguomind_assistant/api/api_service.dart';
...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin
{
  ...
  Future<void> sendRequestToOpenAI(String userInput) async {
    ...
    try {
      ...
      final response = await APIService().requestOpenAI(userInput, modeOpenAI, 2000);
      ...
      if (responseAvailable != null) {
        ...
        } else if (modeOpenAI == "image") {
          if (responseAvailable["data"] != null && responseAvailable["data"].length > 0) {
            setState(() {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  "No image data received from OpenAI API.",
                ),
              ),
            );
          }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "API response is null or invalid.",
            ),
          ),
        );
      }
